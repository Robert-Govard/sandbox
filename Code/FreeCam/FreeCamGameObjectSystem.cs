...
public sealed class FreeCamGameObjectSystem : GameObjectSystem<FreeCamGameObjectSystem>, ISceneStage
{
	...
	/// <summary>
	/// Toggle freecam on or off (same as the default hotkey).
	/// </summary>
	public void Toggle()
	{
		IsActive = !IsActive;
	...
	[ConCmd( "sb_freecam" )]
	public static void ToggleFreeCamCommand()
	{
		var system = Game.ActiveScene?.GetSystem<FreeCamGameObjectSystem>();
		if ( system != null )
		{
			system.Toggle();
		}
	...
	void StartFreeCamMode()
	{
		...
		smoothFov = fov = GamePreferences.FreeCamFov;
	...
	{
		var speed = GamePreferences.FreeCamSpeed;
		if ( Input.Down( "duck" ) ) speed = GamePreferences.FreeCamSpeed * 0.1f;
		if ( Input.Down( "run" ) ) speed = GamePreferences.FreeCamSpeed * 6.0f;
		...
		var smoothing = GamePreferences.FreeCamSmoothing;
		smoothPosition = smoothPosition.LerpTo( position, MathF.Pow( RealTime.SmoothDelta * 16.0f, smoothing * 2.0f ) );
		Scene.Camera.WorldPosition = smoothPosition;

		smoothAngles = smoothAngles.LerpTo( angles, MathF.Pow( RealTime.SmoothDelta * 16.0f, smoothing * 2.0f ) );
